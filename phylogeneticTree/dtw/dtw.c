...
double LocalDist(double *a, double*b, int dim, DistMethods fptr, double *wghts)
{
    ...
    for (ii=0;ii<dim;ii++)
    {
        dist+=((*fptr)(a[ii],b[ii]))*wghts[ii];
    }
    return dist;
}

double CosineDist(double *A, double *B, int Vector_Length)
{
    double dot = 0;
    double denom_a = 0; 
    double denom_b = 0;
...
double dtwNd_std(double *x, double*y, MatrixSize*size_x, MatrixSize*size_y, int NFeatDim, double*cost, Config*  myConfig)
{
        ...
        for (j=1;j<NCols;j++)
        {
            ...
            //                         min3(cost[(i-2)*NCols+j-1], cost[(i-1)*NCols+(j-1)], cost[(i-1)*NCols+(j-2)]);
            cost[(i*NCols)+ j] = CosineDist(&x[i*size_x->Ncol],&y[j*size_y->Ncol], NFeatDim) + 
                                    min3(cost[(i-2)*NCols+j-1], cost[(i-1)*NCols+(j-1)], cost[(i-1)*NCols+(j-2)]);
        }
