...
from utilsFunctions import hz2cents,pitchtrackInterp,SAMPLE_NUM_TOTAL
import numpy as np
import csv,json
...
def getScores(score_path,key, dict_score_info, onlyNotes = True):
...
def melodySynthesize(notes_pitch_hz,notes_quarterlength,onlyNotes):
    '''
    ...
    if onlyNotes:
        ...
        melody = melody - np.mean(melody_nonzero)
...
##-- dump json scores
def dumpScore2Json(score_path,score_info_filepath,score_json_filename,onlyNotes=True):

    dict_score_infos = getDictScoreInfo(score_info_filepath)

    for key in dict_score_infos:
        dict_score_info = dict_score_infos[key]
        dict_score_info = getScores(score_path,key, dict_score_info, onlyNotes)
        ...
        pitchtrack_cents = melodySynthesize(notes_pitch_hz,notes_quarterLength,onlyNotes)
